...
BUILD='(^|/)(Makefile|CMakeLists\.txt|Dockerfile(\.[^/]+)?|Containerfile(\.[^/]+)?|docker-compose[^/]*\.ya?ml|setup\.py|setup\.cfg|MANIFEST\.in|tox\.ini|build\.gradle(\.kts)?|settings\.gradle(\.kts)?|gradle\.properties|pom\.xml|WORKSPACE|MODULE\.bazel|BUILD\.bazel|flake\.nix)$|\.cmake$|\.dockerfile$|(^|/)scripts/(release|deploy|publish)'
...
while IFS=$'\t' read -r meta p2 p3; do
  ...
  [ -n "${path:-}" ] || continue
  # Session ephemera — never part of risk routing (review pathspec also excludes
  # these). Skip without counting so queue/handoff noise cannot inflate tier.
  case "$path" in
    HANDOFF.md|*/HANDOFF.md|NEEDS-HUMAN.md|*/NEEDS-HUMAN.md) continue ;;
  esac
  nfiles=$((nfiles + 1))
