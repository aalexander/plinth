...
0. Snapshot the sensitive-path state and record the pre-run commit so the scope check can catch the
   ...
   it is not attributed to the lane:
       ...
       OUT="$(mktemp -t grok-out.XXXXXX)" || { _orc=$?; rm -rf "$SPEC_DIR"; echo "OUT mktemp failed rc=$_orc"; exit 1; }
...
1. Use the **Write tool**, not Bash, to create the file at the literal `SPEC`
   ...
   ACTUAL output in your final message.” A line such as `SPEC_EOF` has no special
...
`plinth hookprobe`; grok 0.2.112: no hook execution (reproduce: plinth hookprobe grok)) does not run the `.claude/`
