...
5. **Record the DELEGATION RECEIPT — a PRECONDITION of `STATUS: complete`.** Run this AFTER
   ...
   covers, so recording it earlier would read as a sensitive-path violation):
       ...
       .plinth/lane-guard.sh delegation codex <the RUN_RC value> <the OUT path> <the BEFORE sha>

   Pass the literal BEFORE sha from step 0 as the 4th argument so the receipt is bound to
   THIS run. It preserves codex's own transcript + exit code as an artifact under
   ...
   HONEST BOUND: the receipt proves a non-empty transcript EXISTS. It does NOT prove which
   model wrote the diff — `model=` is codex's self-report. Binding BEFORE makes a skipped or
